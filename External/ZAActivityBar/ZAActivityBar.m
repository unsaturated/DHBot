...
- (void)setStatus:(NSString *)string {
    ...
    //MMC (Never used) CGFloat stringHeight = 0;
    ...
    if(string) {
        ...
        NSDictionary *fontAttributes = @{
                                     ...
                                     NSForegroundColorAttributeName : self.stringLabel.textColor};
        
        CGRect stringSize = [string boundingRectWithSize:CGSizeMake(width, 300)
                                                 options:NSStringDrawingUsesLineFragmentOrigin
                                              attributes:fontAttributes
                                                 context:nil];
        
        stringWidth = stringSize.size.width;
        //MMC (Never used) stringHeight = stringSize.size.height;
...
- (UIView *) actionIndicatorView {
    ...
    if (!actionIndicatorView) {
        ...
        pill.path = [UIBezierPath bezierPathWithOvalInRect:actionIndicatorView.bounds].CGPath;
        //MMC (Fix possible Core Foundation memory leak) pill.path = CGPathCreateWithEllipseInRect(actionIndicatorView.bounds, nil);
