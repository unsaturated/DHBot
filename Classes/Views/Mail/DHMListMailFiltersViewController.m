...
#import "DHMListMailFiltersViewController.h"
#import "DHMController.h"
#import "DHMApi.h"
#import "DHMListMailFiltersDataItem.h"

@interface DHMListMailFiltersViewController ()

@end

@implementation DHMListMailFiltersViewController
...
{
    if(_mCommand == nil)
        _mCommand = [[DHMApi sharedInstance] commandFromCommandName:DHM_MAIL_LIST_FILTERS];
...
{    
    if(self.command != nil)
    {
        ...
        RKResponseDescriptor* response = [DHMListMailFiltersDataItem responseDescriptorForREST];
        ...
        [op setCompletionBlockWithSuccess:^(RKObjectRequestOperation *operation, RKMappingResult *mappingResult) {
            ...
            DHMListMailFiltersDataItem* first = ((DHMListMailFiltersDataItem*)queryResult);
            
            if([DHMBaseData isSuccess:first])
            {
                // Success, so get the "actual" data embedded within the first result
                NSArray* apiDataArray = ((DHMListMailFiltersDataItem*)queryResult).data;
...
{
    static NSString *CellIdentifier = @"listFilterCell";
    ...
    if([self.commandResultArray count] > 0)
    {
        ...
        DHMListMailFiltersDataItem* item = [self.commandResultArray objectAtIndex:indexPath.row];
        
        [cell.textLabel setText:item.address];
        [cell.detailTextLabel setText:[NSString stringWithFormat:@"%@, %@, %@: \"%@\"", item.rank, item.action, item.filterOn, item.filter]];
    }
    return cell;
}
