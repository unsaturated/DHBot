...
#import "DHMListMySqlDbViewController.h"
#import "DHMController.h"
#import "DHMApi.h"
#import "DHMListMySqlDbDataItem.h"

@interface DHMListMySqlDbViewController ()

@end

@implementation DHMListMySqlDbViewController
...
{
    if(_mCommand == nil)
        _mCommand = [[DHMApi sharedInstance] commandFromCommandName:DHM_SQL_LIST_DBS];
...
{    
    if(self.command != nil)
    {
        ...
        RKResponseDescriptor* response = [DHMListMySqlDbDataItem responseDescriptorForREST];
        ...
        [op setCompletionBlockWithSuccess:^(RKObjectRequestOperation *operation, RKMappingResult *mappingResult) {
            ...
            DHMListMySqlDbDataItem* first = ((DHMListMySqlDbDataItem*)queryResult);
            
            if([DHMBaseData isSuccess:first])
            {
                // Success, so get the "actual" data embedded within the first result
                NSArray* apiDataArray = ((DHMListMySqlDbDataItem*)queryResult).data;
...
{
    static NSString *CellIdentifier = @"dbNameCell";
    ...
    if([self.commandResultArray count] > 0)
    {
        ...
        DHMListMySqlDbDataItem* item = [self.commandResultArray objectAtIndex:indexPath.row];
        
        [cell.textLabel setText:item.database];
        
        // Convert from megabytes to bytes
        NSNumber* diskBytesNumber = [NSNumber numberWithFloat:(item.diskUsageMb.doubleValue * 1000.0 * 1000.0)];
        NSString* disk = [DHMController convertBytesToReadableValue:diskBytesNumber];
        [cell.detailTextLabel setText:[NSString stringWithFormat:@"%@ | %@", item.description, disk]];
    }
    return cell;
}
