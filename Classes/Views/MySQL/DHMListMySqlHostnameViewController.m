...
#import "DHMListMySqlHostnameViewController.h"
#import "DHMController.h"
#import "DHMApi.h"
#import "DHMListMySqlHostnameDataItem.h"

@interface DHMListMySqlHostnameViewController ()

@end

@implementation DHMListMySqlHostnameViewController
...
{
    if(_mCommand == nil)
        _mCommand = [[DHMApi sharedInstance] commandFromCommandName:DHM_SQL_LIST_HOSTS];
...
{    
    if(self.command != nil)
    {
        ...
        RKResponseDescriptor* response = [DHMListMySqlHostnameDataItem responseDescriptorForREST];
        ...
        [op setCompletionBlockWithSuccess:^(RKObjectRequestOperation *operation, RKMappingResult *mappingResult) {
            ...
            DHMListMySqlHostnameDataItem* first = ((DHMListMySqlHostnameDataItem*)queryResult);
            
            if([DHMBaseData isSuccess:first])
            {
                // Success, so get the "actual" data embedded within the first result
                NSArray* apiDataArray = ((DHMListMySqlHostnameDataItem*)queryResult).data;
...
{
    static NSString *CellIdentifier = @"dbHostCell";
    ...
    if([self.commandResultArray count] > 0)
    {
        ...
        DHMListMySqlHostnameDataItem* item = [self.commandResultArray objectAtIndex:indexPath.row];
        
        [cell.textLabel setText:item.domain];
        [cell.detailTextLabel setText:item.home];
    }
    return cell;
}
