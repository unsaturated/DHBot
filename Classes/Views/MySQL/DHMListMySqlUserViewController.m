...
#import "DHMListMySqlUserViewController.h"
#import "DHMController.h"
#import "DHMApi.h"
#import "DHMListMySqlUsersDataItem.h"

@interface DHMListMySqlUserViewController ()

@end

@implementation DHMListMySqlUserViewController
...
{
    if(_mCommand == nil)
        _mCommand = [[DHMApi sharedInstance] commandFromCommandName:DHM_SQL_LIST_USERS];
...
{    
    if(self.command != nil)
    {
        ...
        RKResponseDescriptor* response = [DHMListMySqlUsersDataItem responseDescriptorForREST];
        ...
        [op setCompletionBlockWithSuccess:^(RKObjectRequestOperation *operation, RKMappingResult *mappingResult) {
            ...
            DHMListMySqlUsersDataItem* first = ((DHMListMySqlUsersDataItem*)queryResult);
            
            if([DHMBaseData isSuccess:first])
            {
                // Success, so get the "actual" data embedded within the first result
                NSArray* apiDataArray = ((DHMListMySqlUsersDataItem*)queryResult).data;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([self.commandResultArray count] > 0)
    {
        ...
        DHMListMySqlUsersDataItem* item = [self.commandResultArray objectAtIndex:indexPath.row];
        
        [cell.textLabel setText:item.username];
        [cell.detailTextLabel setText:[NSString stringWithFormat:@"DB: %@", item.database]];
    }
    return cell;
}
