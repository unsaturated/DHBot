...
#import "DHMListDomainsViewController.h"
#import "DHMApi.h"
#import "DHMController.h"
#import "DHMListDomainsDataItem.h"

@interface DHMListDomainsViewController ()

@end

@implementation DHMListDomainsViewController
...
{
    if(_mCommand == nil)
        _mCommand = [[DHMApi sharedInstance] commandFromCommandName:DHM_DOM_LIST_HOSTED];
...
- (void)executeCommand
{
    if(self.command != nil)
    {
        ...
        RKResponseDescriptor* response = [DHMListDomainsDataItem responseDescriptorForREST];
        ...
        [op setCompletionBlockWithSuccess:^(RKObjectRequestOperation *operation, RKMappingResult *mappingResult) {
            ...
            DHMListDomainsDataItem* first = ((DHMListDomainsDataItem*)queryResult);
            
            if([DHMBaseData isSuccess:first])
            {
                // Success, so get the "actual" data embedded within the first result
                NSArray* apiDataArray = ((DHMListDomainsDataItem*)queryResult).data;
...
{
    static NSString *CellIdentifier = @"listDomainCell";
    ...
    if([self.commandResultArray count] > 0)
    {
        ...
        DHMListDomainsDataItem* item = [self.commandResultArray objectAtIndex:indexPath.row];
